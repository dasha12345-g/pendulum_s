...
namespace pendulum_s
{
    public partial class Form5 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            alpha10Degrees = double.Parse(textBox7.Text);
            alpha20Degrees = -alpha10Degrees;
            buttonClickCount++;
            if (alpha10Degrees == -alpha20Degrees)
            {
                double g = 9.81;
                totalTime = double.Parse(textBox5.Text);
                points = 1000;
                double mass = 0.05;
                double length = 1;
                omega1 = Math.Sqrt(g / length);
                omega2 = Math.Sqrt(g / length + 2 * 0.3 * Math.Pow(0.6, 2) / (mass * Math.Pow(length, 2)));
                double alpha10 = alpha10Degrees * Math.PI / 180;
                double alpha20 = alpha20Degrees * Math.PI / 180;
                ...
                dataGridView1.Rows[rowIndex].Cells[3].Value = omega2.ToString("0.00");
                ...
                listBox1.Items.Add($"Частота: {omega2.ToString("0.00")} рад/с");
            }
